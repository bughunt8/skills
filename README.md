...
## Vendored engineering rigor: pstack

The [`skills/pstack/`](./skills/pstack/) collection is imported verbatim from
...
[`skills/pstack/unslop/`](./skills/pstack/unslop/SKILL.md) is a **mandatory** import. It strips AI
tells from any writing this repository's skills produce, and the sync script fails the build if it
ever goes missing. Apply it to every user-facing sentence.

Four upstream skills are deliberately not imported (`tdd`, `teach`, `reflect` collide with local
skills of the same name; `bro` needs an external repository at runtime). The reasoning is recorded
in [`skills/pstack/ATTRIBUTION.md`](./skills/pstack/ATTRIBUTION.md) rather than left to guesswork.
...
**459 skills** across 21 directories. Counts and licence pins on this page are generated by [`scripts/generate_index.py`](./scripts/generate_index.py) and verified in CI, so they cannot drift from the tree.
