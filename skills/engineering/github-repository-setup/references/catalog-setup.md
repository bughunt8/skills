...
The initial consolidated plan and complex-setup stop gate in
`planning.md` take precedence over proceeding directly through this catalog.
...
This profile excludes Claude. Do not generate `CLAUDE.md` or `Claude.md`,
Claude settings, or Claude integration targets from any upstream preset.
Use `AGENTS.md` for agent instructions; preserve unrelated existing files.
...
- Pin every downloaded upstream template to a full commit SHA, resolving a release tag to its commit,
